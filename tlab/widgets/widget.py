...
def envset(name, default):
    ...
    if name in os.environ:
        return os.environ[name].lower() not in [
...
def _json_to_widget(x, obj):
    if isinstance(x, dict):
        ...
        return [_json_to_widget(v, obj) for v in x]
            ...
            'IPY_MODEL_') and x[10:] in Widget._active_widgets:
    ...
    def __call__(self, *args, **kwargs):
        ...
        for callback in self.callbacks:
            try:
                ...
                ip = get_ipython()
                if ip is None:
                    self.log.warning("Exception in callback %s: %s",
                                     callback,
                                     e,
                                     exc_info=True)
                else:
...
def _show_traceback(method):
    ...
    def m(self, *args, **kwargs):
        try:
            ...
            ip = get_ipython()
            if ip is None:
                self.log.warning("Exception in widget method %s: %s",
                                 method,
                                 e,
                                 exc_info=True)
            else:
...
class WidgetRegistry:
    ...
    def register(self, model_module, model_module_version_range, model_name,
                 view_module, view_module_version_range, view_name, klass):
        """Register a value"""
    ...
    def get(self, model_module, model_module_version, model_name, view_module,
            view_module_version, view_name):
        """Get a value"""
                    ...
                    for view_module, vv in sorted(vm.items()):
                        for view_version, vn in sorted(vv.items()):
                            for view_name, widget in sorted(vn.items()):
                                yield (model_module, model_version, model_name,
                                       view_module, view_version,
                                       view_name), widget
...
def register(widget):
    ...
    Widget._widget_types.register(w['_model_module'].default_value,
                                  ...
                                  w['_view_name'].default_value, widget)
    return widget
...
class Widget(LoggingHasTraits):
    ...
    def _handle_control_comm_msg(cls, msg):
        ...
        if method == 'request_states':
            ...
            cls._control_comm.send(dict(method='update_states',
                                        states=full_state,
